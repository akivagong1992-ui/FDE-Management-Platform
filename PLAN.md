...
### **Phase 5 — 系统用户手册**（所有编程完成后交付）
- [ ] 模块功能说明（按 admin 侧栏顺序逐项）
  - 首页 / 项目和客户管理 / 派单和工时管理 / 利润管理 / 成本和支出管理 /
    项目效率管理 / FDE知识库 / 培训管理 / 关键项目复盘管理 / 系统设置
  - 工程师端门户：我的派单 / 我的工时 / 我的支出申请
- [ ] 用户角色与权限矩阵（lead / admin / pm / finance / engineer 看到什么、能改什么）
- [ ] **驾驶舱降本金额（口径 C）完整计算逻辑** — 必含：
  - 有收入项目：收款门槛、effective_bench = min(报价, 实收)、savings 公式
  - 无收入项目：完成态门槛（closing / archived）、value_created 公式
  - cancelled 状态如何移出
  - 实际操作流程：销售拍报价 → 立项 → 驾驶舱不动；真做事付费 → 还是不动；
    财务确认回款（status=received）→ 按 min(报价,实收) − 实际成本 加；
    跑单 → 状态改 cancelled → 立即移出（即使曾收过款）
- [ ] 工时加权规则（香港工作日 1.0× / 工作日晚上 + 非工作日 1.5×）
- [ ] 派单接 / 拒 / 对话留痕流程
- [ ] 支出审批流（工程师提交 → admin 批准/拒绝 + 理由）
- [ ] 驾驶舱 6 个 Tab 各自展示什么、数据从哪来
- [ ] 常见操作 FAQ：新员工入职、项目跑单、客户回款分期、培训费如何报销等
