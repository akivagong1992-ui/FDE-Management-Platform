...
- **支出分类字典**（**8 类**）：
  - `material` — 耗材（线缆/工具/设备/物料）
  - `subcontract` — **对外分包高级服务**（特殊技术服务、安全测试、专业设计、第三方审计）
  - `temp_labor` — 临时人力补充（短期外援工时）
  - `license` — 第三方平台/许可证（云资源、SaaS、软件 license）
  - `travel` — 差旅/外勤报销
  - `training` — **外部培训费**（培训学费、考试报销，v0.3.x 加）
  - `other` — 其他（不在以上分类的开销，需在描述里说明）
  - `outsource_engineer` — **外包工程师支出** ⭐（v0.5.0 加）— Vendor 用 VSF 钱付给工程师/劳务公司的部分，是 vendor 端最大开销项；进入 A 口径 `team_margin = Σ VSF − Σ 全部支出`，未录入会让 margin 偏高
- **统一流程**：申请 → 审批（lead/finance 单级 approve/reject）→ paid 标记
- **每笔支出必须挂项目**：否则成本无法归集；vendor 角色提交时额外填 `vendor_id` 关联自家公司
