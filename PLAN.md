...
| A4 | **结算/利润口径** | 三层成本透视：①电信付Vendor服务费 ②Vendor真实人工成本（估算/披露）③传统外包模式对标 | 仅单层 |
| A5 | **货币** | 港币 HKD 为主 | 多币种 |
...
- 项目立项：名称、需求方（电信内部部门 / 外部合同方）、起止、负责 PM、预算
- 状态机：`立项 → 进行中 → 验收 → 收尾 → 归档`
- 里程碑、变更记录
- **风险与合规子维度**（合并 D）：风险登记（技术/进度/成本/合规）、合规事件（PDPO 数据保护、母公司合规检查）、安全事故
- 项目文档暂存（与 §4.7 知识资产联动；项目归档时关键文档转入资产库）
- 项目列表 / 看板 / 甘特图
- **给驾驶舱**：在管项目数、按时完成率、项目地图分布、零事故天数
...
### 4.3 利润管理（README §3 维度 2）— **三层成本透视**
- **项目预算**：人工服务费预算 + 外部支出预算 + 杂费 + 含税
- **收入登记**：内部转移定价 / 外部合同金额 / 分期到账
- **第一层 — Vendor服务费**（实际支付）：按合同/订单/月度结算单录入
- **第二层 — Vendor真实人工成本**（估算或协议披露）：用于计算 Vendor 毛利水位、判断议价空间
- **第三层 — 传统外包模式对标**：基于历史项目均价 / 行业基准 / 同类外包报价单 估算"如果走老外包路子"要花多少
- **项目毛利** = 收入 − Vendor服务费 − 外部支出 − 杂费分摊
- **节省金额** = 传统外包估算 − 实际成本（**驾驶舱最 brag 指标**）
- **Vendor 性价比分析**：服务费 / 真实成本 比、议价记录
- **团队损益**：月度、季度滚动
- **给驾驶舱**：累计毛利、累计节省金额、Vendor 性价比榜
...
| Tab | 内容 |
|---|---|
| 1 · 总览 | 4 个核心 KPI（在管项目数、累计毛利、团队规模、累计节省金额）+ 香港项目地图 + 月度毛利曲线 |
| 2 · 项目地图 | 香港地图标点，点击/轮播显示项目卡片（项目名、PM、状态、毛利） |
| 3 · 利润对比 ⭐核心 brag | 本季度 vs 上季度毛利、内化 vs 传统外包估算节省金额、Vendor性价比榜 |
...
Project                    项目
  ...
  └─ OutsourceBenchmark    传统外包模式估算（三层透视第三层）
...
ExternalExpense            外部支出项（统一抽象，含类型字段）
  ├─ ExpenseType           支出分类字典（耗材/分包/临时人力/许可/差旅）
  ├─ ExpenseRequest        申请单
  ├─ ExpenseApproval       审批流
...
ProjectRevenue             项目收入（内部计价/外部合同）
ProjectCost                项目成本（Timesheet→服务费 + ExternalExpense 自动汇总）
ProjectMargin              项目毛利（视图/物化）
SavingsCalc                节省金额计算（基于 OutsourceBenchmark - 实际）
...
- [ ] **Vendor 档案** CRUD
- [ ] **工程师档案** CRUD（归属 Vendor）+ 技能矩阵 + 外部证书（B 字段铺底）
- [ ] **培训记录** CRUD（4.8 基础）
- [ ] 需求方 NeedParty 档案
...
- [ ] 工时录入（手填 + Excel 导入）
- [ ] 项目看板/列表
- **交付**：能"建一个 Vendor、登记几个工程师、立一个项目、派人、记工时"

### **Phase 2 — 利润 + 外部支出 + 知识资产铺底** (3~4 周)
- [ ] 项目预算编制
- [ ] 项目收入登记
- [ ] **三层成本透视**：Vendor 服务费录入 + 真实人工成本（估算/披露字段）+ 传统外包模式对标输入
- [ ] **外部支出管理**：5 类支出统一申请审批流（耗材/分包/临时人力/许可/差旅）
- [ ] 供应商管理（外部支出侧）
- [ ] 项目成本自动归集 → 项目毛利 → **节省金额计算**
- [ ] 月度团队损益
- [ ] **知识资产库基础**（4.7 铺底）：分类字典、上传/检索、保密分级
- **交付**：每个项目赚不赚钱、节省多少、知识资产可归档
